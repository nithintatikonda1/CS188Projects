...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        if (len(self.getLegalActions(state)) == 0):
            return None
        max_q_value = (-2000000000, None)
        for action in self.getLegalActions(state):
            if self.getQValue(state, action) > max_q_value[0]:
              max_q_value = (self.getQValue(state, action), action)
            elif self.getQValue(state, action) == max_q_value[0]:
              max_q_value = random.choice([max_q_value,(self.getQValue(state, action), action)])
        return max_q_value[1]
